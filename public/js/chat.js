...
function _showRecUI(on) {
  ...
  const recCancel  = document.getElementById("recCancelBtn")
  const recSend    = document.getElementById("recSendBtn")
  ...
  if (on) {
    ...
    recCancel.style.display = "flex"
    recSend.style.display   = "flex"
    msgInput.style.display  = "none"
  } else {
    micBtn.style.display    = "flex"
    recBar.style.display    = "none"
    recCancel.style.display = "none"
    recSend.style.display   = "none"
...
async function toggleInlineRecord() {
  ...
  try {
    ...
    showToast("❌ لا يمكن الوصول للميكروفون — تحقق من الإذن في المتصفح")
    return
  }
  _recChunks  = []
  _recSeconds = 0
  ...
  _recTimerInt = setInterval(() => {
    _recSeconds++
    document.getElementById("recTimer").textContent = _fmtTime(_recSeconds)
    if (_recSeconds >= 120) stopAndSendRecord() // حد أقصى دقيقتان
  }, 1000)
}

function cancelInlineRecord() {
  if (_mediaRecorder) { try { _mediaRecorder.stop() } catch {} }
  if (_recStream)     { _recStream.getTracks().forEach(t => t.stop()) }
  clearInterval(_recTimerInt)
  _mediaRecorder = null; _recChunks = []; _recStream = null
  _showRecUI(false)
}
...
async function stopAndSendRecord() {
  ...
  await new Promise(resolve => {
    ...
    try { _mediaRecorder.stop() } catch { resolve() }
  })
  if (_recStream) _recStream.getTracks().forEach(t => t.stop())
  _showRecUI(false)

  if (_recChunks.length === 0 || _recSeconds < 1) {
    showToast("⚠️ التسجيل قصير جداً")
    _mediaRecorder = null; _recChunks = []; _recStream = null
    return
  }
